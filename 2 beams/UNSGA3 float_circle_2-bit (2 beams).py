...
x_r2, y_r2, z_r2 = d2 * sin(theta_r2) * cos(phi_r2), d2 * sin(theta_r2) * sin(phi_r2), d2 * cos(theta_r2)
...
r_r2 = np.sqrt((x_r2 - x_u) ** 2 + (y_r2 - y_u) ** 2 + z_r2 ** 2)
...
class BinaryMatrixProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for i, value in enumerate(x):
            if value < pi/4 and value >= 0:
                x[i] = 0
            elif value < pi/2 and value >= pi/4:
                x[i] = pi/2
            elif value < 3*pi/4 and value >= pi/2:
                x[i] = pi
            else:
                x[i] = 3*pi/2
        # print(x)
...
plt.grid(True)
plt.savefig('out7_0-Evolution of Fitness.png', dpi=300)
plt.show()
...
plt.colorbar()
plt.savefig('out7_1-Phase matrix.png', dpi=300)
plt.show()
...
# Display the plot
plt.savefig('out7_2-Radiation patterns.png', dpi=300)
plt.show()
...
# plt.savefig('out7_3-3dB Area.png', dpi=300)
plt.show()
...
plt.axis('off')
plt.title('Over -3dB Area')
# plt.savefig('out7_6-3dB Area (only).png', dpi=300)
plt.show()
...
# plt.savefig('out7_4-Masks.png', dpi=300)
plt.show()
...
# plt.savefig('out7_5-Check SLL.png', dpi=300)
plt.show()
...
# File path
csv_file_path = 'out7_UNSGA3 float_circle_2-bit (2 beams).csv'
