...
class BinaryMatrixProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        E_L1_1 = ((u_-u_0)*cos(theta_r) + (v_-v_0)*sin(theta_r))**2 / sin(Theta_hp/2)**2
        E_L1_2 = ((u_-u_0)*sin(theta_r) - (v_-v_0)*cos(theta_r))**2 / (sin(Theta_hp/2) * cos(theta_r))**2
        E_L = E_L1_1 + E_L1_2
        ...
        for p in range(0, len(r_p_vect)):
            for q in range(0, len(r_q_vect)): 
                if E_L[p, q] <= 1 and values[p, q] < -3:
                    F_L.append((values[p, q] - (-3))**2)

        F_L_sum = sum(F_L) / (len(F_L)**2)

        # Second objective function and Upper mask
        E_U1_1 = ((u_-u_0)*cos(theta_r) + (v_-v_0)*sin(theta_r))**2 / sin(Theta_fn/2)**2
        E_U1_2 = ((u_-u_0)*sin(theta_r) - (v_-v_0)*cos(theta_r))**2 / (sin(Theta_fn/2) * cos(theta_r))**2
        E_U = E_U1_1 + E_U1_2
        ...
        for p in range(0, len(r_p_vect)):
            for q in range(0, len(r_q_vect)): 
                if E_U[p, q] > 1 and values[p, q] > SLL:
