...
for n in range(0, N):
    for m in range(0, M):
        ...
        r_total[n, m] = r_t[n, m] + r_r[n, m]
        r_max[n, m] = max(r_total) - r_total[n, m]
        phi_Gamma[n, m] = (2 * pi * r_max[n, m]) / wavelength

        # # The phase shift calculated from GA
        # phi_Gamma[n, m] = -k * ((m + 1) * dx * sin(theta_r) * cos(phi_r) + \
        #                         (n + 1) * dy * sin(theta_r) * cos(phi_r))
        
        # Make sure the phase shift between 0 and 2pi
        phi_Gamma[n, m] = np.mod(phi_Gamma[n, m], 2 * pi)
...
# theta_inf, theta_sup = 0, 85
# phi_inf, phi_sup = 0, 360
# pas_r, pas_theta, pas_phi = 1, 5, 5

theta_inf, theta_sup = 0, 85.5
phi_inf, phi_sup = 0, 342
pas_r, pas_theta, pas_phi = 1, 4.5, 18
